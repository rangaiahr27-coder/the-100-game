...
function randomTimeframe() {
  ...
  const span = 5 + Math.floor(Math.random() * 11); // 5–15 years
  const latestStart = CURRENT_YEAR - span;
  const startSeason = ALL_TIME_START + Math.floor(Math.random() * (latestStart - ALL_TIME_START + 1));
  return { startSeason, endSeason: startSeason + span, label: `${startSeason}–${startSeason + span}`, allTime: false };
}

function generateChallenge() {
  const category = ALL_STATS[Math.floor(Math.random() * ALL_STATS.length)];
  return { category, timeframe: randomTimeframe() };
}
...
function parseInnings(ip) {
  ...
  return (parseInt(s.slice(0, dot)) || 0) + (parseInt(s.slice(dot + 1)) || 0) / 3;
}

// Single-season leaders (counting stats)
async function fetchSingleSeasonLeaders(leaderCategory, season) {
  ...
  return (data?.leagueLeaders?.[0]?.leaders ?? []).map(l => ({
    id: l.person?.id,
...
// Full season stats (rate stats — includes IP, AB, etc.)
...
// Counting stats: sum each player's totals — keep ALL players (no slice) for full rank lookup
async function fetchMultiYearLeaderboard(category, startSeason, endSeason) {
  ...
  // Keep ALL players sorted — rank is their position in this full list
...
// Rate stats: IP-weighted or AB-weighted average — keep ALL qualifying players
  ...
  seasonData.forEach(players => {
    players.forEach(p => {
      ...
      if (stat === 'era' || stat === 'whip') {
        ...
        if (ip > 0 && val > 0) { acc[p.id].weight += ip; acc[p.id].numerator += val * ip; }
      } else if (stat === 'avg') {
        const ab = parseInt(p.atBats) || 0;
        const val = parseFloat(p.avg) || 0;
        if (ab > 0 && val > 0) { acc[p.id].weight += ab; acc[p.id].numerator += val * ab; }
      } else if (stat === 'ops') {
        const pa = parseInt(p.plateAppearances) || 0;
        const val = parseFloat(p.ops) || 0;
        if (pa > 0 && val > 0) { acc[p.id].weight += pa; acc[p.id].numerator += val * pa; }
      }
    });
  });

  const minWeight = (stat === 'era' || stat === 'whip')
    ? Math.max(60, yearSpan * 20)
    : Math.max(200, yearSpan * 60);

  let ranked = Object.entries(acc)
    .filter(([, d]) => d.weight >= minWeight && d.numerator > 0)
    .map(([id, d]) => ({ id: parseInt(id), name: d.name, value: d.numerator / d.weight }));

  if (cacheReady) ranked = ranked.filter(p => playerCache.has(p.id));

  ranked.sort((a, b) => higherBetter ? b.value - a.value : a.value - b.value);

  // Keep ALL players — rank is their actual position
...
async function fetchLeaderboard(category, timeframe) {
  ...
  if (isRateStat) return fetchMultiYearRateLeaderboard(category, timeframe.startSeason, timeframe.endSeason);
...
async function buildPlayerCache() {
  console.log('[mlbApi] Building player cache…');
  ...
  results.forEach(players => players.forEach(p => { if (p.id && p.fullName) playerCache.set(p.id, p.fullName); }));
  cacheReady = true;
  console.log(`[mlbApi] Cache ready — ${playerCache.size} players`);
}
...
async function searchPlayers(query) {
  ...
  if (!cacheReady) {
    const r = await fetch(`${MLB_BASE}/sports/1/players?season=2024&fields=people,id,fullName`);
    if (!r.ok) return [];
    const d = await r.json();
    return (d?.people ?? []).filter(p => p.fullName.toLowerCase().includes(q)).slice(0, 10).map(p => ({ id: p.id, name: p.fullName }));
  }
  const results = [];
  for (const [id, name] of playerCache) {
    if (name.toLowerCase().includes(q)) { results.push({ id, name }); if (results.length >= 10) break; }
  }
  return results;
}
